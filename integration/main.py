...
class Main:
    ...
    class basics:
        ...
        def version_output(self):
            _output_eq("invoke --version", f"Invoke {__version__}\n")
    ...
    class ptys:
        ...
        def pty_puts_both_streams_in_stdout(self):
            if WINDOWS:
                return
            os.chdir("_support")
            err_echo = f"{sys.executable} err.py"
            command = f"echo foo && {err_echo} bar"
    ...
    class parsing:
        def false_as_optional_arg_default_value_works_okay(self):
            ...
            for argstr, expected in (
                ...
                _output_eq(
                    f"inv -c parsing foo {argstr}", expected + "\n"
                )
