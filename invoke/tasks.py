...
def call(task, *args, **kwargs):
    ...
    Useful for setting up :doc:`task dependencies </concepts/execution>`. It's
    actually just a convenient wrapper around the `.Call` class, which may be
    used directly instead if desired.
